...
async function startServer() {
  ...
  // SEO Middleware for Blog Pages (Pre-injects metadata for Google and social crawlers)
  app.get(['/blog', '/blog/:id'], async (req, res, next) => {
    ...
    try {
      ...
      let imageUrl = "https://odishaexamprep.com/og-image.jpg"; // Default fallback
      const canonicalUrl = `${req.protocol}://${req.get('host')}${req.originalUrl}`;
      let schemaJson = "";

      // If it is a blog post detail page, fetch details from database
      if (blogId) {
