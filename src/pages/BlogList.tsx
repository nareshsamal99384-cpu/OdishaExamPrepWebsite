      ...
      <div className="py-8 sm:py-12 md:py-16 px-4 sm:px-6 lg:px-8 relative">
        <div className="max-w-5xl mx-auto space-y-8 sm:space-y-12 mt-4 sm:mt-8">
          ...
          <div className="text-center space-y-3 sm:space-y-4">
            <motion.div {...fadeSlideDown} transition={{ ...fadeSlideDown.transition, duration: durations.slow }} className="w-16 h-16 sm:w-20 sm:h-20 bg-brand-100 rounded-[1.5rem] flex items-center justify-center mx-auto mb-4 sm:mb-6 shadow-sm border border-brand-200/50">
              <BookOpen className="w-8 h-8 sm:w-10 sm:h-10 text-brand-600" />
            </motion.div>
            <motion.h1 {...fadeSlideDown} transition={{ ...fadeSlideDown.transition, duration: durations.slow, delay: 0.1 }} className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 tracking-tight leading-tight">OEP <br className="sm:hidden" /><span className="premium-text-gradient">Knowledge Base</span></motion.h1>
            <motion.p {...fadeSlideDown} transition={{ ...fadeSlideDown.transition, duration: durations.slow, delay: 0.2 }} className="text-sm sm:text-base md:text-lg text-slate-500 font-medium max-w-2xl mx-auto px-2">Latest insights, exam strategies, and announcements tailored for Odisha aspirants.</motion.p>
          </div>

          {/* Search */}
          <motion.div {...fadeSlideDown} transition={{ ...fadeSlideDown.transition, duration: durations.slow, delay: 0.3 }} className="relative max-w-xl mx-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                  {filteredBlogs.map((blog, idx) => (
                    <motion.div
                      key={blog.id}
                      {...fadeSlideDown}
                      transition={{ ...fadeSlideDown.transition, duration: durations.slow, delay: 0.4 + ((idx % 6) * 0.1) }}
                    ...
                    >
                      <Link to={`/blog/${blog.id}`} className="group block h-full bg-white rounded-3xl overflow-hidden shadow-lg shadow-slate-200/50 border border-slate-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300">
                        <div className="h-[55%] sm:h-[60%] bg-slate-100 overflow-hidden relative">
                          {blog.icon ? (
                            <img src={getDirectImageUrl(blog.icon)} alt={blog.name} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                          ) : (
